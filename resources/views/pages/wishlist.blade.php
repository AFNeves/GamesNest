...
@section('content')
    ...
    @else
        <div class="flex flex-grow space-x-4 px-4 w-full max-w-[1500px]">
            <div class="flex flex-col flex-grow items-center justify-start p-4 w-full bg-gray-600 rounded-xl space-y-4">
                @foreach($items as $item)
                    <div class="flex w-full bg-gray-800 rounded-xl p-4 justify-between items-center min-h-44">
                        ...
                        <div class="flex flex-col flex-grow justify-between text-base mx-4 h-full p-4">
                        ...
                        <form method="POST" action="{{ route('cart.store') }}" class="flex items-center space-x-4">
                            ...
                            <button type="submit" class="font-audio text-lg hover:text-primary cst-tr rounded-lg p-6">Add to Cart</button>
                        </form>

                        <div class="flex flex-col justify-evenly h-full ml-4">
                            <form action="{{ route('wishlist.destroy') }}" method="POST" class="self-center">
                                @csrf
                                @method('DELETE')
                                <input type="hidden" name="product_id" value="{{ $item->id }}">
                                <button type="submit">
                                    <img src="{{ asset('images/icons/closed_heart.svg') }}" alt="Remove from Wishlist" class="w-10 h-10 hover:scale-95 cst-tr">
                                </button>
                            </form>
                        </div>
                    </div>
